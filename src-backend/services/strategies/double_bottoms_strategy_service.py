...
from models.request_models.strategies.double_bottoms_strategy_request import DoubleBottomsStrategyRequest
from services.strategies.base_strategy_service import BaseStrategyService

pd.options.mode.chained_assignment = None


class DoubleBottomsStrategyService(BaseStrategyService):
  ...
  @property
  def strategy_request(self) -> DoubleBottomsStrategyRequest:
    return ModelUtils.get_obj(DoubleBottomsStrategyRequest(), self._strategy_request)
  ...
  def _do_preparations(self) -> None:
    ...
    for symbol in self.symbols:
      ...
      self.prices = self._calc_service.append_double_bottoms(
          prices=self.prices,
          index=[symbol.id],
          ...
          double_bottoms_diff=self.strategy_request.double_bottoms_diff)
  ...
  def _is_valid_request(self) -> bool:
    return self.strategy_request \
        ...
        and self.strategy_request.double_bottoms_diff >= 0.01 \
        and self.strategy_request.double_bottoms_diff <= 5
  ...
  def _has_entry_conditions(self, symbol_id: int, current_date: date) -> bool:
    ...
    has_double_bottoms: bool = (current_row.loc[AppConsts.CUSTOM_COL_DOUBLE_BOTTOMS] == 1)
    if has_double_bottoms:
      self._target[symbol_id] = {}
      self._target[symbol_id]['target_price'] = current_row.loc[AppConsts.CUSTOM_COL_DOUBLE_BOTTOMS_TARGET_PRICE]
      self._target[symbol_id]['stop_loss'] = current_row.loc[AppConsts.CUSTOM_COL_DOUBLE_BOTTOMS_STOP_LOSS]
    return has_double_bottoms
  ...
  def _has_exit_conditions(self, symbol_id: int, current_date: date) -> bool:
    ...
    return True  # is_exit - Custom Double Bottoms Exit Strategy that gets better results.
